...
이 프로젝트의 핵심은 MuJoCo에서 만든 합성 이미지로 인식 모델을 학습하는 것이 아닙니다. 핵심은 **우리 실제 리얼월드에서 촬영한 타겟 데이터로 학습한 YOLO 모델이, 실제 환경과 최대한 유사하게 만든 MuJoCo 회의실 월드에서도 똑같이 추론되는지 확인하는 것**입니다.

즉, 이 프로젝트는 다음 질문에 답하기 위한 실험입니다.

```text
실제 카메라 데이터로 학습한 YOLO 모델이
MuJoCo 안에 재현한 회의실/테이블/타겟 환경에서도
target을 안정적으로 검출할 수 있는가?

검출이 된다면 bbox confidence, bbox 위치/크기 안정성은 어느 정도인가?

그 bbox 정보를 이용해 고무줄 발사대의 yaw/pitch 조준 보정과
자동 발사까지 연결할 수 있는가?
```

따라서 이 프로젝트는 **real-world trained perception model을 sim 환경에 가져와 성능을 확인하는 real-to-sim perception transfer 실험**입니다.
...
1. 실제 환경에서 타겟 이미지를 촬영합니다.
2. 실제 이미지 데이터로 YOLO 타겟 검출 모델을 학습합니다.
3. 학습된 모델을 ONNX로 변환해 `yolo_model/target_yolo11s_640_best.onnx`로 사용합니다.
4. MuJoCo 안에 실제 회의실 사진과 비슷한 테이블, 유리벽, 스크린, 의자, 조명, 타겟을 구현합니다.
5. 포신에 달린 `aim_camera`가 실제 카메라와 비슷한 시야를 갖도록 위치와 fovy를 조정합니다.
6. MuJoCo 렌더링 화면에 색감, 밝기, bloom, blur 후처리를 적용해 실제 웹캠 느낌에 가깝게 만듭니다.
7. 실제 데이터로 학습된 YOLO 모델을 MuJoCo Aim Camera 프레임에 그대로 적용합니다.
8. 검출된 bbox 정보를 이용해 조준 보정 데이터셋을 수집합니다.
9. bbox feature에서 `delta_yaw`, `delta_pitch`를 예측하는 조준 보정 모델을 학습합니다.
10. `O` 키 입력 시 YOLO 검출, 반복 보정, 고무줄 1회 발사를 자동으로 수행합니다.

## 현재 성능 요약

MuJoCo Aim Camera에서 수집한 5cm 전체 테이블 조준 데이터셋 기준:
...
이 성능은 MuJoCo 데이터셋 위에서 bbox 기반 조준 보정량을 예측한 결과입니다. 실제 자동 발사 명중률은 다음 단계에서 여러 타겟 위치를 자동 순회하며 별도 평가할 예정입니다.
...
Ridge regression은 선형 회귀에 규제를 추가한 모델입니다.  
일반 선형 회귀가 입력 feature에 가중치를 곱해 출력값을 예측한다면, ridge regression은 예측 오차를 줄이면서도 가중치가 지나치게 커지지 않도록 벌점을 줍니다.
...
ridge regression:
예측 오차를 줄이되, 큰 가중치에는 벌점을 줌
```

이 프로젝트에서 ridge regression을 사용한 이유는 다음과 같습니다.

- `numpy`만으로 가볍게 학습/저장/로드할 수 있습니다.
- 5만 row 이상의 조준 데이터에 대해 빠르게 학습됩니다.
- 가중치가 과하게 커지는 것을 막아 예측이 안정적입니다.
- 첫 자동조준 모델로 bbox 기반 조준 보정이 가능한지 빠르게 검증하기 좋습니다.

모델의 역할은 절대 서보모터 각도를 직접 예측하는 것이 아니라, 현재 상태에서 얼마나 더 움직여야 하는지인 보정량을 예측하는 것입니다.

```text
입력:
YOLO bbox 중심 오차, bbox 크기, bbox 면적, bbox confidence, 현재 yaw/pitch

출력:
delta_yaw, delta_pitch
```

Aim Camera는 포신에 붙어 있기 때문에 포신이 움직이면 화면도 같이 움직입니다. 그래서 `target 위치 -> 고정 서보 각도`를 바로 외우는 방식보다, `현재 화면 -> 현재 상태 기준 보정량`을 예측하는 방식이 더 적합합니다.

현재 모델은 원본 feature뿐 아니라 feature의 2차항과 feature끼리의 곱도 입력에 포함합니다. 이를 통해 완전히 직선적인 관계뿐 아니라 bbox 크기, 현재 각도, 화면 위치가 함께 만드는 비선형적인 관계도 일부 표현할 수 있게 했습니다.
